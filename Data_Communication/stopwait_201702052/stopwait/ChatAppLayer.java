...
public class ChatAppLayer implements BaseLayer {
    ...
    private void waitACK() { // Receive ACK
        while (ackChk.size() <= 0) {
            try {
                Thread.sleep(10);
...
/* ################################################################################################################# */
    private void fragSend(byte[] input, int length) {
        ...
        for(frameCount=1; frameCount<maxLen; frameCount++) {
        	waitACK();
        	if(frameCount+1 == maxLen && length%10 == 0) {
        		// 전송 데이터가 10으로 나누어 떨어질 때,
        		m_sHeader.capp_type = (byte) (0x03);
        	}
        	System.arraycopy(input, frameCount*10, bytes, 0, 10);
        	bytes = objToByte(m_sHeader, bytes, 10);
        	this.GetUnderLayer().Send(bytes, bytes.length);
        }
...
/* ################################################################################################################# */
 ...
 /* ################################################################################################################# */
    public synchronized boolean Receive(byte[] input) {
        ...
        if(tempType == 0x00) {
            m_sHeader.capp_data = RemoveCappHeader(input, input.length);
            this.GetUpperLayer(0).Receive(m_sHeader.capp_data);
        }
        else{
        	...
        	if(tempType == 0x01) {
        		...
        		tempBytes = RemoveCappHeader(input, input.length);
        		System.arraycopy(tempBytes, 0, fragBytes, (fragCount+1)*10, size);
        		if(tempType == 0x03) {
        			this.GetUpperLayer(0).Receive(fragBytes);
        		}
        	}
        }
        this.GetUnderLayer().Send(null, 0); // send ACK
        return true;
    }
/* ################################################################################################################# */
