...
//	#### FileAppLayer Send File Methods 
...
/* ###################################################################################### */	
	public synchronized boolean Receive(byte[] input) {
		...
		// Data
		else if(temp_type == 0x01) {
			if(chkAddr(input) || (isBroadcast(input)) || !isMyPacket(input)) {
				m_sHeader.enet_data = RemoveEthernetHeader(input, input.length);
				this.GetUpperLayer(0).Receive(m_sHeader.enet_data);
				this.Send(null, 0);	// send ACK
				return true;
			}
		}
